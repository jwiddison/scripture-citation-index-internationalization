...
def removeSpanClassLine(string_to_clean):
    ...
    for index, location in enumerate(list(matches)):
        temp_string += string_to_clean[beg_offset + location.start() : location.end() - end_offset]
        # Add 1 to account for 0-indexed index
        if index + 1 < len(list(matches)):
            temp_string += break_tag

    allCaps = re.finditer('XXXXXXXXXXXXXXXXXXXXXX(.*?)YYYYYYY', temp_string)
    for index, location in enumerate(list(allCaps)):
        temp_string = temp_string[:location.start()] + '<span class="allCaps">' + temp_string[location.start() + len('<span class="allCaps">'):]
        temp_string = temp_string[:location.end() - len('</span>')] + '</span>' +  temp_string[location.end():]
